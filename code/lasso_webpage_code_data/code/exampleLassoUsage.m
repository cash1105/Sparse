...
if ( exist('../data/sampleDataForLasso.mat') == 2 )
else
    disp('sampleDataForLasso.mat is not on current matlab path!');
    z = [];
    return;
end

% load example data
data = load('../data/sampleDataForLasso.mat');
...
plot( data.y ,'r.-','LineWidth',1);hold on;
plot( data.X * data.beta, 'b-', 'LineWidth',2);

% fit Lasso using optimal lambda determined using cross validation
sL = solveLasso( data.y, data.X, 1 );
plot( data.X * sL.beta, 'g-', 'LineWidth', 2 );
...
plot( data.beta, 'bd-', 'LineWidth', 1, 'MarkerFaceColor','c', 'MarkerSize',10 );hold on;
